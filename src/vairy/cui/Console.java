...
import vairy.gui.MainAlermPanel;
...
public class Console {
	...
	private enum ScriptSub{
		NONE("終了",null),
		PROGRAMSTART("プログラム開始時",AlermScriptType.PROGRAMSTART),
		TIMERSTART("タイマー動作開始時",AlermScriptType.TIMERSTART),
		TIMEREXEC("タイマー満了時",AlermScriptType.TIMEREXEC),
		TIMERSTOP("タイマー動作停止時",AlermScriptType.TIMERSTOP),
		PROGRAMEND("プログラム終了時",AlermScriptType.PROGRAMEND),

		;
		public final String disptext;
		public final AlermScriptType type;
		private ScriptSub(final String text,final AlermScriptType type){
			this.disptext = text;
			this.type = type;
		}
	}
	...
	private enum DriveMode{
		ALERM("アラーム",AlermMode.ALERM),
		TIMER("タイマー",AlermMode.TIMER),

		;
		public final String disptext;
		public final AlermMode mode;
		private DriveMode(final String text,AlermMode mode){
			this.disptext = text;
			this.mode = mode;
		}
	}
	...
	private Integer doEditDate(String buff, DateSub datesub2) {
		Integer rtn = 0;
		Calendar alerm = null;
		AlermMode mode = editbean.getMode();

		if(AlermMode.ALERM == mode){
			alerm = editbean.getAlerm();
		}else if (AlermMode.TIMER == mode) {
...
//					ctrlmode = ControlMode.EDITDATESUB;
	...
	private CalendarBar createCalendarBar(AlermBean target) {
		...
		if (AlermMode.TIMER == target.getMode()) {
		...
		}else if (AlermMode.ALERM == target.getMode()) {
